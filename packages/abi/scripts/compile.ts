// @ts-check
/// <reference path="../tsconfig.scripts.json" />
...
import { tokenVerificationKeys } from "../src/vk/index.js";
...
export const ${networkId}: ChainVerificationKeysList =  ` +
      ...
      await fs.writeFile(`./${networkId}.ts`, vkString);
    }
  });
}
...
async function main() {
  ...
  if (!chain || !["mainnet", "devnet"].includes(chain)) {
    ...
    await compileContracts(chain as blockchain);
  }
}
