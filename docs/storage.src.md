---
title: STORAGE
category: 6749c4dba3a7a4005bae1197
hidden: false
order: 6
---



# STORAGE

## Index

### Classes

- [FieldOption](storagesrcclassfieldoption)
- [OffChainList](storagesrcclassoffchainlist)
- [OffChainListBase](storagesrcclassoffchainlistbase)
- [OffchainMap](storagesrcclassoffchainmap)
- [OffchainMapOption](storagesrcclassoffchainmapoption)
- [Storage](storagesrcclassstorage)
- [UInt64Option](storagesrcclassuint64option)
- [Whitelist](storagesrcclasswhitelist)
- [WhitelistedAddress](storagesrcclasswhitelistedaddress)

### Interfaces

- [IndexedMapSerialized](storagesrcinterfaceindexedmapserialized)
- [OffChainMapEntry](storagesrcinterfaceoffchainmapentry)
- [OffchainMapSerialized](storagesrcinterfaceoffchainmapserialized)

### Type Aliases

- [IndexedMapSerializedJson](storagesrctypealiasindexedmapserializedjson)
- [IpfsHash](storagesrctypealiasipfshash)
- [OffChainMapEntries](storagesrctypealiasoffchainmapentries)

### Functions

- [bigintFromBase56](storagesrcfunctionbigintfrombase56)
- [bigintFromBase64](storagesrcfunctionbigintfrombase64)
- [bigintToBase56](storagesrcfunctionbiginttobase56)
- [bigintToBase64](storagesrcfunctionbiginttobase64)
- [createIpfsURL](storagesrcfunctioncreateipfsurl)
- [deserializeIndexedMerkleMap](storagesrcfunctiondeserializeindexedmerklemap)
- [fieldFromBase56](storagesrcfunctionfieldfrombase56)
- [fieldFromBase64](storagesrcfunctionfieldfrombase64)
- [fieldToBase56](storagesrcfunctionfieldtobase56)
- [fieldToBase64](storagesrcfunctionfieldtobase64)
- [fromBase](storagesrcfunctionfrombase)
- [loadIndexedMerkleMap](storagesrcfunctionloadindexedmerklemap)
- [parseIndexedMapSerialized](storagesrcfunctionparseindexedmapserialized)
- [pinJSON](storagesrcfunctionpinjson)
- [saveIndexedMerkleMap](storagesrcfunctionsaveindexedmerklemap)
- [serializeIndexedMap](storagesrcfunctionserializeindexedmap)
- [sleep](storagesrcfunctionsleep)
- [toBase](storagesrcfunctiontobase)
